...
export default function HeroGrid() {
  ...
  return isMobile ? <MobileNetworkGrid /> : <DesktopNetworkGrid />;
}

function MobileNetworkGrid() {
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      nodes.forEach(n => {
        if (n.isAccent) {
          const pulse = 0.5 + 0.5 * Math.sin(time * 0.002 + n.idlePhaseX);
          const grd = ctx.createRadialGradient(n.x, n.y, 0, n.x, n.y, n.radius * 4);
          grd.addColorStop(0, `rgba(255,61,46,${0.65 * baseAlpha * pulse})`);
